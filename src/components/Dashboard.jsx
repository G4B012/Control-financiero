...
export default function Dashboard({ state, setState, onLogout, onResetUser }) {
  ...
  const savingsMonth = state.savings
  ...
  const debtMonth = state.debtPayments
  ...
  const donutBase = totalIncome > 0 ? totalIncome : 0;
  const donutData = [
    { name: "Fijos", value: donutBase ? fixedTotalActual / donutBase : 0 },
    { name: "Variables", value: donutBase ? varTotalActual / donutBase : 0 },
    { name: "Ahorro", value: donutBase ? savingsMonthTotal / donutBase : 0 },
    { name: "Deuda", value: donutBase ? debtMonthTotal / donutBase : 0 },
  ...
  return (
    <div className="min-h-screen p-5 md:p-8 bg-gradient-to-br from-[#fff4f4] via-white to-[#fff0ef]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mt-5">
          ...
          <div className="cardBody space-y-2">
            <div>
              ...
              <RowInput
                ...
                type="number"
                value={state.salary}
                onChange={(v) => setState((s) => ({ ...s, salary: Number(v || 0) }))}
        ...
        >
          <div className="cardHeader flex items-center justify-between">
            <div className="label">Distribución (% ingresos)</div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              {donutData.map((d) => (
                <div key={d.name} className="badge justify-between">
                  <span>{d.name}</span>
                  <span>{(d.value * 100).toFixed(1)}%</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="cardBody space-y-3">
            ...
            <div className="rowCard space-y-2">
              <div className="flex justify-between text-sm font-semibold text-wine2">
                <span>Progreso</span>
                <span>{(goalPct * 100).toFixed(1)}%</span>
              </div>
...
<div className="card lg:col-span-4">
  ...
  <div className="cardBody space-y-3">
    ...
    <button className="btnGhost w-full" onClick={() => addExpenseRow("variable")}>
          ...
          <div className="cardBody space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="rowCard pop-in">
                ...
                <div className="text-wine text-xl font-extrabold mt-1">
                  {money(Math.max(0, debtTotal - debtPaidTotal), currency)}
                </div>
              </div>
            </div>
      ...
      <div className="mt-6 text-xs text-wine2/60">
        Datos guardados localmente en este navegador por usuario.
      </div>
    </div>
  );
}
