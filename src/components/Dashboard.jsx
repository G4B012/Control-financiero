...
export default function Dashboard({ state, setState, onLogout, onResetUser }) {
  ...
  const prevExp = sum(
    state.expenses.filter((e) => e.month === prev),
    (e) => Number(e.amount || 0)
  );
  ...
  const upsertExpense = (type, idx, patch) => {
    setState((s) => {
      ...
      const row =
        list[idx] || { id: newId(), month, type, category: "", budget: "", amount: "", note: "" };
      const updated = { ...row, ...patch };

      const masterIdx = rows.findIndex((r) => r.id === updated.id);
      if (masterIdx >= 0) rows[masterIdx] = updated;
      ...
      if (cat) {
        ...
        const existing = tpls.find(
          (t) => t.category === cat && (t.endMonth == null || month < t.endMonth)
        );
        if (bud > 0) {
          if (existing) {
            existing.budget = bud;
  ...
  const addExpenseRow = (type) => {
    setState((s) => ({
      ...s,
      expenses: [...s.expenses, { id: newId(), month, type, category: "", budget: "", amount: "", note: "" }],
    }));
  };
  ...
  return (
    <div className="min-h-screen p-5 md:p-8 bg-gradient-to-br from-[#fff4f4] via-white to-[#fff0ef]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mt-5">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className="card lg:col-span-3">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: 0.1 }} className="card lg:col-span-3">
          <div className="cardHeader flex items-center justify-between">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: 0.15 }} className="card lg:col-span-3">
          <div className="cardHeader flex items-center justify-between">
            ...
            <div className="mt-3 space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-extrabold text-wine">
                  {change == null ? "—" : `${(change * 100).toFixed(0)}%`} {changeOk == null ? "" : (changeOk ? "🟢" : "🔴")}
                </span>
              </div>
              <div className="flex items-center justify-between text-sm">
              ...
              {[...savingsMonth, { id: null, month, date: "", amount: "", note: "" }].map((r, idx) => (
                <div key={r.id || idx} className={"rowCard " + (r.id ? "pop-in" : "")}>
                  <div className="grid grid-cols-12 gap-2 items-center">
                    ...
                    <div className="col-span-5">
                      <RowInput className="text-right" type="number" value={r.amount} onChange={(v) => upsertSavings(idx, { id: r.id || newId(), amount: v })} placeholder="Monto" />
                    </div>
                ...
                <div key={r.id || idx} className={"rowCard " + (r.id ? "pop-in" : "")}>
                  <div className="grid grid-cols-12 gap-2 items-center">
                    ...
                    <div className="col-span-3">
                      <RowInput className="text-right" type="number" value={r.budget} onChange={(v) => upsertExpense("fixed", idx, { id: r.id || newId(), budget: v })} placeholder="0" />
                    </div>

                    <div className="col-span-3">
                      <RowInput className="text-right" type="number" value={r.amount} onChange={(v) => upsertExpense("fixed", idx, { id: r.id || newId(), amount: v })} placeholder="0" />
                    </div>

                    <div className="col-span-2 flex items-center gap-2">
                      <RowInput small value={r.note} onChange={(v) => upsertExpense("fixed", idx, { id: r.id || newId(), note: v })} placeholder="Nota" />
                      <span className="text-lg">{semaforo(Number(r.budget || 0), Number(r.amount || 0))}</span>
              ...
              {rowsFor("variable").map((r, idx) => (
                <div key={r.id || idx} className={"rowCard " + (r.id ? "pop-in" : "")}>
                  <div className="grid grid-cols-12 gap-2 items-center">
                    ...
                    <div className="col-span-3">
                      <RowInput className="text-right" type="number" value={r.budget} onChange={(v) => upsertExpense("variable", idx, { id: r.id || newId(), budget: v })} placeholder="0" />
                    </div>

                    <div className="col-span-3">
                      <RowInput className="text-right" type="number" value={r.amount} onChange={(v) => upsertExpense("variable", idx, { id: r.id || newId(), amount: v })} placeholder="0" />
                    </div>

                    <div className="col-span-2 flex items-center gap-2">
                      <RowInput small value={r.note} onChange={(v) => upsertExpense("variable", idx, { id: r.id || newId(), note: v })} placeholder="Nota" />
                      <span className="text-lg">{semaforo(Number(r.budget || 0), Number(r.amount || 0))}</span>
                      {r.id ? <button className="btnGhost px-2" onClick={() => removeExpense(r.id)}>✕</button> : <span className="text-wine2/30">—</span>}
                    </div>
                  </div>
          ...
          <div className="cardBody space-y-3">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                ...
                <RowInput className="text-right" type="number" value={state.debt?.total || 0} onChange={(v) => setState((s) => ({ ...s, debt: { ...s.debt, total: Number(v || 0) } }))} placeholder="0" />
              </div>
            </div>
              ...
              {[...debtMonth, { id: null, month, date: "", amount: "", note: "" }].map((r, idx) => (
                <div key={r.id || idx} className={"rowCard " + (r.id ? "pop-in" : "")}>
                  <div className="grid grid-cols-12 gap-2 items-center">
                    ...
                    <div className="col-span-5">
                      <RowInput className="text-right" type="number" value={r.amount} onChange={(v) => upsertDebtPay(idx, { id: r.id || newId(), amount: v })} placeholder="Monto" />
                    </div>
                    <div className="col-span-3 flex items-center gap-2">
